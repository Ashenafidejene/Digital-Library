...
async getUsers(
    ...
    return apiService.get<User[]>(`/admin/users?${params.toString()}`);
  },
   /**
   * Fetch borrowing records
   */
  async getBorrowingRecords(): Promise<ApiResponse<RawBorrowingRecord[]>> {
    return apiService.get<RawBorrowingRecord[]>(`/admin/borrowing`);
  },
  /**
   * Add new admin
   */
