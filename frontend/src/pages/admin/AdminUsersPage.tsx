...
const AdminUsersPage: React.FC = () => {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      if (selectedClearance) params.append('clearance', selectedClearance);
      params.append('limit', '50');
      params.append('includeBorrowing', 'true'); // Request borrowing data

      const url = `http://localhost:3000/api/v1/admin/users?${params.toString()}`;

      // Fetch users data first
      const usersResponse = await fetch(url);

      if (!usersResponse.ok) {
        throw new Error(`Backend error: ${usersResponse.status} - ${usersResponse.statusText}`);
      }
      ...
      // Try to fetch borrowing data, but don't fail if it's not available
      let borrowingRecords: RawBorrowingRecord[] = [];
      try {
        const borrowingResponse = await fetch('http://localhost:3000/api/v1/admin/borrowing');
        if (borrowingResponse.ok) {
          const borrowingData: BorrowingApiResponse = await borrowingResponse.json();
          borrowingRecords = borrowingData.success && Array.isArray(borrowingData.data)
            ? borrowingData.data
            : [];
        }
      } catch (borrowingError) {
        console.warn('Could not fetch borrowing data:', borrowingError);
        // Continue without borrowing data
      }

      // Enhance users with borrowing data
      const enhancedUsers = users.map((user: User) => {
        // Find user's borrowing records
        const userBorrowings: RawBorrowingRecord[] = borrowingRecords.filter(
          (record: RawBorrowingRecord) => record.userId === user.id
        );

        // Calculate current borrowings
        const currentBorrowings: BorrowedBook[] = userBorrowings
          .filter((record: RawBorrowingRecord) => record.status === 'borrowed' || record.status === 'active')
          .map((record: RawBorrowingRecord) => ({
            id: record.id,
            ...
            status: record.dueDate && calculateDaysLeft(record.dueDate) < 0 ? 'overdue' : 'active',
        ...
        const pendingRequests: BookingRequest[] = userBorrowings
          .filter((record: RawBorrowingRecord) => record.status === 'pending')
          .map((record: RawBorrowingRecord) => ({
            id: record.id,
            bookId: record.bookId,
            bookTitle: record.bookTitle,
        ...
        // Calculate stats
        const overdueBooks = currentBorrowings.filter(b => b.status === 'overdue').length;
        const clearanceStatus = overdueBooks > 0 ? 'blocked' :
                               pendingRequests.length > 0 ? 'pending' : 'clear';
      ...
      const stats = {
        ...
        totalBorrowedBooks: enhancedUsers.reduce((sum: number, u: User) => sum + u.borrowedBooks, 0),
        totalOverdueBooks: enhancedUsers.reduce((sum: number, u: User) => sum + (u.overdueBooks || 0), 0),
        totalPendingRequests: enhancedUsers.reduce((sum: number, u: User) => sum + (u.pendingBookings || 0), 0),
      };
          ...
          <div className="flex justify-between items-center mt-6 pt-4 border-t border-neutral-200 dark:border-neutral-700">
            ...
            <div className="flex space-x-3">
              ...
              {isClear && (
                <button
                  onClick={() => {
                    alert('Clearance certificate generated successfully!');
                    onClose();
                  }}
