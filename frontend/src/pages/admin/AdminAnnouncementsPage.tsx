...
const AdminAnnouncementsPage: React.FC = () => {
  ...
  const fetchAnnouncements = useCallback(async () => {
    try {
      ...
      const url = `http://localhost:3000/api/v1/admin/announcements?${params.toString()}`;
  ...
  const handleAddAnnouncement = async (announcementData: Partial<Announcement>) => {
    try {
      const url = 'http://localhost:3000/api/v1/admin/announcements';
      ...
      if (data.success) {
        ...
        setAnnouncements(prevAnnouncements => [data.data, ...prevAnnouncements]);
  ...
  const handleDeleteAnnouncement = async (announcementId: string) => {
    if (window.confirm('Are you sure you want to delete this announcement?')) {
      try {
        const url = `http://localhost:3000/api/v1/admin/announcements/${announcementId}`;
  ...
  const handleToggleStatus = async (announcementId: string, currentStatus: string) => {
    ...
    try {
      const url = `http://localhost:3000/api/v1/admin/announcements/${announcementId}/status`;
        ...
        setAnnouncements(prevAnnouncements => 
          prevAnnouncements.map(announcement => 
            announcement.id === announcementId 
              ? { ...announcement, status: newStatus as Announcement['status'] }
              : announcement
          )
        );
        alert(`Announcement ${newStatus === 'published' ? 'published' : 'unpublished'} successfully`);
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Add Announcement Modal - TODO: Create AddAnnouncementModal component */}
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-neutral-800 p-6 rounded-lg max-w-md w-full mx-4">
            <h3 className="text-lg font-medium mb-4">Add Announcement Modal</h3>
            <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-4">
              AddAnnouncementModal component needs to be created
            </p>
            <button
