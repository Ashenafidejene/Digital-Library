...
const AdminSettingsPage: React.FC = () => {
  ...
  const saveSettings = async () => {
    try {
      ...
      toast.error(t('messages.error.saveSettings'));
    } finally {
      setSaving(false);
    }
  };
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (response.success && response.data) {
          const { library, borrowing, notifications, system } = response.data;
          if (library) setLibrarySettings(library);
          if (borrowing) setBorrowingSettings(borrowing);
        ...
        toast.error(t('messages.error.fetchSettings'));
      } finally {
        setLoading(false);
      }
    };
